...
@section('content')
	<div class="container-fluid">
		...
		<div class="d-sm-flex align-items-center justify-content-between mb-4">
			<h1 class="h3 mb-0 text-dark font-weight-bold">Tambah Penjualan</h1>
		</div>
				...
				<form action="{{ route('penjualan.store') }}" method="post" enctype="multipart/form-data">
					...
					<div class="row mb-2">
						<div class="col-6">
							...
							<input type="date" class="form-control form-control-solid mb-2" name="document_date" value="{{old('document_date')}}">
							@error('document_date') <span>{{$message}}</span> @enderror
						</div>
						<div class="col-6">
							<label>Masa Pajak</label>
							<input type="date" class="form-control form-control-solid mb-2" name="tax_period" value="{{old('tax_period')}}">
							@error('tax_period') <span>{{$message}}</span> @enderror
						</div>
						<div class="col-6">
							<label>Tahun Pajak</label>
							<input type="date" class="form-control form-control-solid mb-2" name="tax_year" value="{{old('tax_year')}}">
							@error('tax_year') <span>{{$message}}</span> @enderror
						</div>
					</div>
					<hr>
					...
					<div class="row mb-2">
						...
						<div class="col-6">
							...
							<input type="number" class="form-control form-control-solid mb-2 hitung" id="up" name="unit_price" value="{{old('unit_price')}}">
							@error('unit_price')
								<span>{{$message}}</span>
							@enderror
						</div>
						<div class="col-6">
							...
							<input type="number" class="form-control form-control-solid mb-2 hitung" id="am" name="amount" value="{{old('amount')}}">
							@error('amount')
								<span>{{$message}}</span>
							@enderror
						</div>
						...
						<div class="col-6">
							<label>Diskon</label>
							<input type="number" class="form-control form-control-solid mb-2 hitung" id="ds" name="discount" value="{{old('discount')}}">
							@error('discount')
								<span>{{$message}}</span>
							@enderror
						</div>
